...
export async function fetchCrons(): Promise<CronRow[]> {
  const res = await fetch(`${BASE}/crons`, { headers: auth, signal: AbortSignal.timeout(25_000) });
...
export async function deleteCronRemote(id: string): Promise<void> {
  const res = await fetch(`${BASE}/crons/${id}`, { method: "DELETE", headers: auth, signal: AbortSignal.timeout(25_000) });
