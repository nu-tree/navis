...
import { internalError, json, withAppAuth } from "./respond.js";
...
//
// 인증은 앱 API 와 같은 APP_API_TOKEN — 공개돼 있으면 아무나 크론을 강제 발동시켜
// 토큰을 태울 수 있다. 다만 클레임 덕분에 중복 호출 자체는 무해하다
...
export function handleSchedulerTick(req: Request): Promise<Response> {
  return withAppAuth(
    req,
    "[scheduler] 틱 실패:",
    async () => {
      ...
      console.log(
        `[scheduler] 틱 완료 — ran=${result.ran.length} skipped=${result.skipped.length} ` +
